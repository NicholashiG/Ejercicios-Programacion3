...
public class Laberinto{
	...
	public static void main(String[] args) {
		...
		m.resuelve(8, 1); 								// ahora, introducimos la entrada (S) en las coordenadas (8,1)
														// y llamamos al algoritmo
		System.out.println(m.imprimirLaberinto()); 	    // imprimimos el laberinto ya resuelto (si tiene soluciÛn)
	}
    ...
    private boolean paso(int x, int y) 
    {
    	 ...
    	 if (laberinto[x][y]=='X'){ // si hemos llegado a X quiere decir que hemos encontrado soluciÛn
    	 ...
    	 // caso intermedio, por lo tanto, que empezamos a recorrer o todavÌa no hemos llegado a nada
    	 laberinto[x][y]='*'; // marcamos la posiciÛn como visitada (si es la primera, en las coordenadas x e y 
    	 ...
    	 // si no hemos encontrado la soluciÛn en estos cuatros movimientos, volvemos atr·s, aunque hay que
    	 // considerar que en este punto, todas las llamadas recursivas han finalizado. Si en ninguna de ellas
    	 // se ha conseguido el Èxito, entonces el algoritmo termina y devuelve false.
    	 laberinto[x][y]='f'; // en el caso de no ser el resultado, se marca con +. Ya fue pisado
    	 return false; // vuelta atr·s si la soluciÛn no se encuentra aquÌ

       
    }
	
    private String imprimirLaberinto() { // imprimiremos nuestra soluciÛn. Debido a que la clase Arrays no tiene implementado
        String salida = "";    // un mÈtodo toString para arrays bidimensionales, lo programamos a mano
        for (int x=0; x<laberinto.length; x++) { // recorremos filas
            ...
            salida += "\n"; // devolvemos esta variable con un salto de lÌnea
        }
        return salida;
    }
